...
Scenario: Launch the Browser then Add User Agent and Operator IP Range
    ...
    Given Set the Headers with "SAPIND2C_VODAFONE" IP range "212.80.167.67"

########   Wifi Flow  #########   
##### User Subscribe Flow######      
Scenario: Delete the DB Record from Packs and DM log table.
	 Then Delete the User "9005001111" records from Packs DB
	 Then Delete "DIAUS_INDIA_DB" the Log Entry for User "349005001111" form DM Log Table
	 	
Scenario: HIT IndetifyMe API and Identify the user status.
	 When Navigate to url "Spaind2c_vodafone_IdentifyMe_API_URL"
	 Then Validate is Logo present
	  And Validate is Banner present
	 Then Enter the MSISDN "9005001111"
	 ...
	 Then Validate is PoweredBy Logo present
	 Then Validate footer links text and link url
     Then Get the OTP Value for MSISDN "9005001111" from DB and Enter the OTP
     Then Click on Confirm Button
     Then Validate Redirection url Parameter "status=SUCCESS&u=xxxxxxxxx&opr=VODAFONE"
     Then Validate the User Status "NOT_ACTIVE" using "Spaind2c_vodafone_Check_Status_API" Check Status API
...
Scenario: User Subscribe flow With out MSISDN Header
    When Navigate to url "Spaind2c_vodafone_Subscribe_API_URL_CallBack"
    Then Validate is Logo present
	And  Validate is Banner present
	Then Enter the MSISDN "9005001111"
    ...
    Then Get the OTP Value for MSISDN "9005001111" from DB and Enter the OTP
    Then Click on Confirm Button
    Then Validate User "349005001111" DM Log Details for Activity Type "48" from DIAUS DB "DIAUS_DB_URL"
    Then Get the current domain Cookies value
	Then Click on Subscribe Button in CG Page
	Then Validate User "349005001111" DM Log Details for Activity Type "103" from DIAUS DB "DIAUS_DB_URL"
    Then Validate Subscribe thankyou messages and Status of User "9005001111"
    Then Validate User "349005001111" DM Log Details for Activity Type "55" from DIAUS DB "DIAUS_DB_URL"
    Then Validate User "349005001111" DM Log Details for Activity Type "53" from DIAUS DB "DIAUS_DB_URL"
    Then Validate User "349005001111" DM Log Details for Activity Type "60" from DIAUS DB "DIAUS_DB_URL"
    Then Validate is PoweredBy Logo present
	Then Validate footer links text and link url
 ...
 Scenario: HIT IndetifyMe API and Identify the user status.
     Then Activate or DeActivate the User "9005001111,VODAFONE_SPAIN_SUB,50.00,Vodafone,ACT" using "Packs_CallBack_API" Packs Callback API
     When Navigate to url "Page_Load_Url"
     When Delete all domain Cookies Value
	 When Navigate to url "Spaind2c_vodafone_IdentifyMe_API_URL"
	 Then Enter the MSISDN "9005001111"
     ...
     Then Click on Confirm Button
     Then Validate Redirection url Parameter "status=SUCCESS&u=xxxxxxxxx&opr=VODAFONE"
     Then Validate the User Status "ACTIVE" using "Spaind2c_vodafone_Check_Status_API" Check Status API

########   Wifi Flow  #########   
##### User Subscribe Flow(Pass Pack Id in Url)######   
Scenario: Delete the DB Record fro Packs and DM log table.
	 Then Delete the User "9005006666" records from Packs DB
	 Then Delete "DIAUS_INDIA_DB" the Log Entry for User "349005006666" form DM Log Table
...
Scenario: HIT IndetifyMe API and Identify the user status.
	 ...
	 Then Validate is Logo present
	  And Validate is Banner present
	 Then Enter the MSISDN "9005006666"
	 Then Validate Cancel and Corfirm Button is Present
	 Then Validate Terms and Conditions and Value
	 Then Validate is PoweredBy Logo present
	 Then Validate footer links text and link url
	 ...
	 Then Validate is PoweredBy Logo present
	 Then Validate footer links text and link url
     Then Get the OTP Value for MSISDN "9005006666" from DB and Enter the OTP
     Then Click on Confirm Button
     Then Validate Redirection url Parameter "status=SUCCESS&u=xxxxxxxxx&opr=VODAFONE"
     Then Validate the User Status "NOT_ACTIVE" using "Spaind2c_vodafone_Check_Status_API" Check Status API
...
Scenario: User Subscribe flow With out MSISDN Header
    When Navigate to url "Spaind2c_vodafone_Subscribe_API_URL_CallBack_WithPackId"
    Then Validate is Logo present
	And  Validate is Banner present
	Then Enter the MSISDN "9005006666"
	Then Validate Subscribe messages
    ...
    Then Get the OTP Value for MSISDN "9005006666" from DB and Enter the OTP
    Then Click on Confirm Button
    Then Validate User "349005006666" DM Log Details for Activity Type "48" from DIAUS DB "DIAUS_DB_URL"
    Then Get the current domain Cookies value
	Then Click on Subscribe Button in CG Page
	Then Validate User "349005006666" DM Log Details for Activity Type "103" from DIAUS DB "DIAUS_DB_URL"
    Then Validate Subscribe thankyou messages and Status of User "9005006666"
    Then Validate User "349005006666" DM Log Details for Activity Type "55" from DIAUS DB "DIAUS_DB_URL"
    Then Validate User "349005006666" DM Log Details for Activity Type "53" from DIAUS DB "DIAUS_DB_URL"
    Then Validate User "349005006666" DM Log Details for Activity Type "60" from DIAUS DB "DIAUS_DB_URL"
    Then Validate is PoweredBy Logo present
	Then Validate footer links text and link url
    Then Click on Ok Button
    When Delete all domain Cookies Value
 ...
 Scenario: HIT IndetifyMe API and Identify the user status.
     Then Activate or DeActivate the User "9005006666,OMAN_PACK_2,50.00,Ooredoo,ACT" using "Packs_CallBack_API" Packs Callback API
     When Navigate to url "Page_Load_Url"
     When Delete all domain Cookies Value
