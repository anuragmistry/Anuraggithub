...
Scenario: Navigate the Portal with DM API.
    When Navigate to url "Page_Load_Url"
    When Delete all domain Cookies Value
	When Navigate to url "Vodafone_DM_URL_RealTime"
...
Scenario: User DM flow via UIM Page(With out MSISDN Header)
	...
	Then Enter the MSISDN "9119999999"
	Then Click on Confirm Button
	Then Validate is Logo present
	 And Validate is Banner present
	Then Validate Re-Send OTP Text Link
	Then Validate Cancel and Corfirm Button is Present
    Then Get the OTP Value for MSISDN "9119999999" from DB and Enter the OTP
    Then Click on Confirm Button
    Then Validate "DIAUS_INDIA_DB" User "9119999999" DM Log details with Action "Landing_Subscription_Page" for DM Subscription flow
    ...
    Then Validate "DIAUS_INDIA_DB" User "9119999999" DM Log details with Action "Click_Subscription_Successfully" for DM Subscription flow
    Then Validate DM task information after redirect with callBack url
    Then Click on Confirm Button
    Then Waiting for few sec....
    Then Validate "DIAUS_INDIA_DB" User "99999998119999999933" DM Log details with Action "Landing_Confirmation_Page" for DM Subscription flow


Scenario: Navigate the Portal with DM API.
    When Navigate to url "Page_Load_Url"
    When Delete all domain Cookies Value
	When Navigate to url "Vodafone_DM_URL_CallBack"
	
Scenario: User DM flow via UIM Page(With out MSISDN Header)
    Then Validate is Logo present
	And  Validate is Banner present
	Then Validate DM task information from DIAUS DB
	Then Enter the MSISDN "9899999991"
	Then Click on Confirm Button
	Then Validate is Logo present
	 And Validate is Banner present
	Then Validate Re-Send OTP Text Link
	Then Validate Cancel and Corfirm Button is Present
    Then Get the OTP Value for MSISDN "9899999991" from DB and Enter the OTP
    Then Click on Confirm Button
    Then Get the current domain Cookies value
    Then Validate is Logo present
	 And Validate is Banner present
 ...
 #   Then Click on Terms and condition check box
    ...
    Then Validate Subscribe thankyou messages and Status of User "9899999991"
    Then Validate DM task information after redirect with callBack url
    Then Click on Confirm Button
